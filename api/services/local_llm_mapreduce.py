"""
Extended version của LanguageModelOllama tích hợp sẵn tokenizer 
để tương thích trực tiếp với MapReduceLLM
"""
...
class LanguageModelOllamaMapReduce(LanguageModelOllama):
    """
    Mở rộng LanguageModelOllama để tương thích với MapReduceLLM
    Thêm tokenizer và format lại output
    
    QUAN TRỌNG: Class này override các phương thức để tương thích với
    MapReduceLLM framework, vốn được thiết kế cho PyTorch models.
    """
    ...
    def __init__(self, model: str, tokenizer_name: str = None, 
        ...
        Args:
            ...
            temperature: Nhiệt độ generation
        ...
        # Auto-detect tokenizer nếu không được chỉ định
        if tokenizer_name is None:
            tokenizer_name = self._auto_detect_tokenizer(model)
            print(f"🔍 Auto-detected tokenizer: {tokenizer_name}")
        
        # Thêm tokenizer (MapReduce cần để đếm tokens)
        print(f"Đang tải tokenizer: {tokenizer_name}...")
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
        
        # Đặt pad_token nếu chưa có (một số tokenizer không có)
        if self.tokenizer.pad_token is None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
        
        # Thêm thuộc tính device để tương thích với MapReduce
        # (MapReduce mong đợi PyTorch model, nhưng ta dùng Ollama API)
        self.device = "cpu"  # Dummy value vì Ollama chạy trên server riêng
    
    def _auto_detect_tokenizer(self, model_name: str) -> str:
        """
        Tự động phát hiện tokenizer phù hợp dựa trên tên model
        
        Args:
            model_name: Tên model Ollama (vd: "shmily_006/Qw3:4b_4bit", "llama3.2:3b")
            
        Returns:
            Tên tokenizer HuggingFace phù hợp
        """
        ...
        # Default: PhoBERT cho tiếng Việt
        print(f"   ⚠️  Không nhận diện được model {model_name}, dùng PhoBERT (tiếng Việt)")
        return "vinai/phobert-base"
    
    def generate(self, query: str) -> Dict:
        """
        Override phương thức generate để trả về format mà MapReduce yêu cầu
        
        Args:
            query: Prompt/câu hỏi
            
        Returns:
            Dict với keys: "text", "answer", "rationale"
        """
        # Gọi hàm generate gốc từ class cha
        response = super().generate(query)
        
        # Format lại theo yêu cầu của MapReduce
        return {
            "text": response,
            "answer": response,
    ...
    def process_chunk(self, chunk: str, query: str) -> Dict:
        """
        Xử lý một chunk văn bản với query
        Phương thức này được gọi bởi MapReduceLLM.map_stage()
        
        Args:
            chunk: Đoạn văn bản cần xử lý
        ...
        if len(tokens) > max_tokens:
            # Cắt bớt chunk để vừa context window
...
class OllamaMapReduceLLM:
    ...
    def process_long_text(self, document: str, query: str) -> Dict:
        ...
        # 3. Collapse stage (nếu cần)
        if len(mapped_results) > 2:
            print("🔄 Collapse stage: Merging results...")
