...
from model.utils.ctc_utils import get_output_with_timestamps, get_output, get_output_with_timestamps_1
...
def endless_decode(args, model, char_dict):    
    ...
    #         "decode": item["decode"]
    #     })
    #     # Vẫn in ra cho dễ theo dõi
    #     print(f"{item['start']} - {item['end']}: {item['decode']}")

    # for item in decode:
    #     start = f"{Fore.RED}{item['start']}{Style.RESET_ALL}"
    #     end = f"{Fore.RED}{item['end']}{Style.RESET_ALL}"
    #     result[f"{start}-{end}"] = item['decode']
